...
import interFaceModuleConfig from "rain-interface-tools/types/interFaceModuleConfig";

/**
 * @type {import('rain-interface-tools/types/interFaceModuleConfig').default}
 * 这是模块化的方式写法 (注意: 模块化和非模块化两种方式, 可以一起用, 没有限制)
 */
let userInterface: interFaceModuleConfig<typeof globalFun> = {
    ...
    interfaceList: {
        // 定义当前模块中的接口列表 (注意: 只有 importsConfigObj() 函数才会对 模块化接口对象处理, 所以如果你使用了模块化接口对象, 就一定要使用 importsConfigObj() 函数)
